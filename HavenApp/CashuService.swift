...
class CashuService: ObservableObject {
    ...
    func mintTokens(quoteId: String, amountSats: UInt64) async throws {
        ...
        await publishTokenEvent(proofs: newProofs)
        await publishHistoryEvent(direction: "in", amount: amountSats, memo: "Mint from Lightning")
    }
    ...
    func meltTokens(quoteId: String, amount: UInt64, feeReserve: UInt64) async throws -> Bool {
        ...
        if paid {
            ...
            for eventId in spentEventIds {
                await deleteTokenEvent(eventId: eventId)
            }
            ...
            if let changeSigs = response.change {
                ...
                if !changeProofs.isEmpty {
                    ...
                    await publishTokenEvent(proofs: changeProofs)
                }
            }

            recalculateBalance()
            saveState()
            await publishHistoryEvent(direction: "out", amount: amount, memo: "Melt to Lightning")
        }
    ...
    func createSendToken(amountSats: UInt64, memo: String? = nil) async throws -> String {
        ...
        await publishHistoryEvent(direction: "out", amount: amountSats, memo: memo ?? "Send ecash token")
        return token
    }
    ...
    func receiveToken(tokenString: String) async throws {
        ...
        await publishTokenEvent(proofs: freshProofs)
        await publishHistoryEvent(direction: "in", amount: totalAmount, memo: "Received ecash token")
    }
    ...
    func publishWalletEvent() async {
        guard let nostrService = await getNostrService() else { return }
    ...
    func publishTokenEvent(proofs: [CashuProof]) async {
    ...
    func publishHistoryEvent(direction: String, amount: UInt64, memo: String?) async {
        guard let nostrService = await getNostrService() else { return }
    ...
    func deleteTokenEvent(eventId: String) async {
        guard let nostrService = await getNostrService() else { return }
    ...
    func loadWalletFromRelays() async {
        ...
        let relayURLs = config.activeBlastrRelays.compactMap { URL(string: $0) }
