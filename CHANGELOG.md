...
> **WoT Media Filtering, FIPS Publishing, Shimmer Placeholders & Connection Reuse**: Media tab now filters by Web of Trust; FIPS overlay network Blossom publishing with auto-detection of nostr-vpn; video thumbnails show an animated shimmer placeholder; event publishing reuses the existing feed socket; tagged-media spam excluded; toolbar menus simplified to always-inline; log-level filtering; macOS keyboard shortcuts; and broad thread-safety fixes.
